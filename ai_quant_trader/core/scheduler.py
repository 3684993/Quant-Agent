        ...
        try:
            ...
            if hasattr(self, 'order_manager') and self.order_manager:
                ...
                for order in managed_orders:
                    ...
                    # 检查是否是反向委托（止盈委托方向与持仓相反）
                    # 不再检查 reduce_only，因为对冲模式不支持
                    if order_side == take_profit_side:
                        # 检查价格是否在合理范围内（布林线边缘）
                        # 如果价格接近，说明已有止盈委托
                        price_diff_pct = abs(order_price - take_profit_price) / take_profit_price * 100
                        
                        if price_diff_pct < 2.0:  # 价格差异在 2% 以内，认为是同一个止盈委托
                            has_take_profit = True
                            logger.debug("[ORDER] 已有止盈委托：%s %.4f @ %.2f (价格差异 %.2f%%)", 
                                       order_side, float(order.get("quantity", 0)), order_price, price_diff_pct)
                            break
